...
def parse_question_file():
    """解析question.txt文件，返回题目数据"""
    question_file_path = os.path.join(os.path.dirname(__file__), '..', 'frontend', 'question.txt')

    if not os.path.exists(question_file_path):
        print(f"错误：找不到题目文件 {question_file_path}")
        return []
...
def init_quiz_questions():
    ...
    if existing_count > 0:
        print(f"题目数据已存在，共 {existing_count} 道题目，跳过初始化")
        return
